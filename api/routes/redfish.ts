    ...
    node.map(async (val) => {
      ...
      if (bmc.status === "success") {
        ...
        if (auth.status === "success") {
          let api_res: any = { status: "error", message: "" }
          if (auth.oem === "Dell") api_res = await dell_clearSel(uri, auth)
          else if (auth.oem === "Supermicro") api_res = await sm_clearSel(uri, auth)
          else if (auth.oem === "HPE") api_res = await hpe_clearSel(uri, auth)
          else
            api_res = {
              status: "error",
  ...
  let response = await Promise.all(
    node.map(async (val) => {
      ...
      if (bmc.status === "success") {
        ...
        if (auth.status === "success") {
          let api_res: any = { status: "error", message: "" }
          if (auth.oem === "Dell") api_res = await dell_resetBmc(uri, auth)
          else if (auth.oem === "Supermicro") api_res = await sm_resetBmc(uri, auth)
          else if (auth.oem === "HPE") api_res = await hpe_resetBmc(uri, auth)
          else
            api_res = {
              status: "error",
              message: "failed to parse OEM from Redfish call",
  ...
  let response = await Promise.all(
    nodes.map(async (val) => {
      ...
      if (bmc.status === "success") {
        ...
        if (auth.status === "success") {
          let api_res: any = { status: "error", message: "" }
          if (auth.oem === "Dell") api_res = await dell_resetNode(uri, auth, pxe)
          else if (auth.oem === "Supermicro") api_res = await sm_resetNode(uri, auth, pxe)
          else if (auth.oem === "HPE") api_res = await hpe_resetNode(uri, auth, pxe)
          else
            api_res = {
              status: "error",
              message: "failed to parse OEM from Redfish call",
