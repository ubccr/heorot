...
export async function dell_resetNode(uri: string, auth: Auth, pxe: string) {
  if (pxe === "true") {
...
export async function sm_resetNode(uri: string, auth: Auth, pxe: string) {
  if (pxe) {
...
export async function hpe_resetNode(uri: string, auth: import("./auth").Auth, pxe: string) {
  if (pxe) {
    const pxe_url = uri + "/redfish/v1/Systems/1"
    const pxe_body = JSON.stringify({ Boot: { BootSourceOverrideTarget: "Pxe" } })
    let pxe_res = await api_request(pxe_url, auth, "PATCH", false, pxe_body)
