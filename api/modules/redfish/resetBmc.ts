...
export async function dell_resetBmc(uri: string, auth: Auth) {
...
export async function sm_resetBmc(uri: string, auth: Auth) {
  const body = JSON.stringify({ ResetType: "GracefulRestart" })
...
export async function hpe_resetBmc(uri: string, auth: Auth) {
