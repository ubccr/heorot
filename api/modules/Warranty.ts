...
export async function warrantyApiReq(serviceTag: string) {
  let token = config.WARRANTY_API_TOKEN
  let warrantyRes: any = await warrantyAPI(serviceTag, token)

  if (warrantyRes.Fault && warrantyRes.Fault.faultcode === "401") {
    let authRes: any = await authAPI()
...
export async function warrantyAPI(serviceTag: string, token: string) {
