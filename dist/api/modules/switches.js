    ...
    if (type === "EOS") {
        ...
        try {
            ...
            if (convertTmp["rpc-reply"].hasOwnProperty("bulk"))
                ...
                tmp = convertTmp["rpc-reply"].data[0];
            if (tmp.hasOwnProperty("system")) {
                ...
                let ifaces = tmp["interfaces-state"][1].interface.map((val) => {
                    let speed = "";
                    if (val.speed[0] === "100000000")
                        speed = "100M";
                    else if (val.speed[0] >= "1000000000")
                        speed = `${val.speed[0] / 1000000000}G`;
                    let vlans = [];
                    if (val["untagged-vlan"] !== undefined)
                        vlans.push(val["untagged-vlan"][0]);
                    if (val["tagged-vlans"] !== undefined)
                        vlans.push(val["tagged-vlans"][0]);
                    return {
...
const parseOldOutput = (data, command, parseType) => {
    if (command.includes("inventory")) {
        ...
        if (stIndex > 0) {
            ...
            tmpST.forEach((e) => e.forEach((val) => {
                if (val.match("[0-9A-Z]{5,7}"))
                    ST = val.match("[0-9A-Z]{5,7}")[0];
            }));
        }
        // return tmp
        return {
            model: notNull(tmp.find((e) => e.find((val) => val.includes("System Type"))), 1),
            version: notNull(tmp.find((e) => e.find((val) => val.includes("Software Version"))), 1),
            serviceTag: ST,
        };
    }
        ...
        let tmp = data.split("\r\n").map((val) => val.split(": "));
        return {
            model: notNull(tmp.find((val) => val[0].includes("Machine Type")), 1),
            name: notNull(tmp.find((val) => val[0].includes("System Name")), 1),
            uptime: notNull(tmp.find((val) => val[0].includes("Up Time")), 1),
        };
    }
        ...
        let macAddressTable = tmp
            ...
            if (val.match("1/[a-z]{1,2}[0-9]{1,2}")) {
                ...
                let port = "";
                if (tmpPort.substring(0, 1) === "g")
                    port = tmpPort.substring(1);
                else if (tmpPort.substring(0, 2) === "xg")
                    port = parseInt(tmpPort.substring(2)) + 48; // might need to change
                else
                    port = tmpPort;
                return {
                    port: port,
        ...
        let mapping = tmp
            ...
            if (val.match("(Gi)|(Te)1/0/[0-9]{1,2}")) {
                ...
                if (tmpSpeed === "100000")
                    speed = "100G";
                else if (tmpSpeed === "40000")
                    speed = "40G";
                else if (tmpSpeed === "10000")
                    speed = "10G";
                else if (tmpSpeed === "1000")
                    speed = "1G";
                else if (tmpSpeed === "100")
        ...
        let mapping = tmp
            ...
            if (val.match("[0-1]/[0-9]{1,2}")) {
                ...
                let speed = "";
                if (tmpSpeed === "100000 Mbit")
                    speed = "100G";
                else if (tmpSpeed === "40000 Mbit")
                    speed = "40G";
                else if (tmpSpeed === "10000 Mbit")
                    speed = "10G";
                else if (tmpSpeed === "1000 Mbit")
                    speed = "1G";
                else if (tmpSpeed === "100 Mbit")
                    speed = "100M";
                else if (tmpSpeed === "10 Mbit")
                    speed = "10M";
                return {
        ...
        let mapping = tmp
            ...
            if (val.length > 1) {
                let port = "";
                if (val[3].match("^Te"))
                    port = val[3].trim().substring(5);
                else if (val[3].match("^Gi"))
                    port = val[3].trim().substring(5);
                else
                    port = val[3].trim();
                return {
...
const notNull = (data, pos = 0) => {
    ...
    if (data !== undefined && data !== null && data[pos] !== undefined)
        return data[pos].trim();
    else
        return null;
};
