...
export default async function dell_badRequestFix(fqdn, auth) {
    ...
    if (res.data.status === 200)
        return { status: "success", message: "Successfully modified iDRAC values" };
    else
        return { status: "error", message: "Error sending redfish request" };
}
